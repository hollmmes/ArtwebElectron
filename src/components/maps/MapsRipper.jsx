...
export default function MapsRipper() {
  ...
  const cityCoord = selectedCity ? cityCoords[selectedCity] : null
  ...
  const handleMapSelectBusiness = (biz) => {
    setSelectedBusiness(biz)
  }

  const progressPercent = progress ? Math.round((progress.current / progress.total) * 100) : 0
  const newCount = results.filter(r => r.is_new).length
  const showMap = !!(selectedCity && cityCoord)
  ...
  return (
    <div className="h-full flex flex-col gap-3 min-h-0">
      ...
      {/* Search Form */}
      <form onSubmit={handleSearch} className={`rounded-xl p-4 space-y-3 shrink-0 ${isDark ? 'bg-slate-900/40 border border-slate-800/60' : 'bg-white border border-gray-200 shadow-sm'}`}>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 relative" ref={districtRef}>
            <button type="button" disabled={!selectedCity}
            ...
            {districtOpen && (
              <div className={`absolute z-[9999] top-full mt-1 left-0 right-0 rounded-lg border shadow-lg max-h-56 overflow-y-auto ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
                <div className={`flex gap-2 px-3 py-2 border-b sticky top-0 ${isDark ? 'bg-slate-900 border-slate-700/60' : 'bg-white border-gray-100'}`}>
                  <button type="button" onClick={() => setSelectedDistricts([...districts])} className="text-xs text-blue-500 hover:text-blue-400">Tümünü seç</button>
                  <span className={`text-xs ${isDark ? 'text-slate-600' : 'text-gray-300'}`}>·</span>
                  <button type="button" onClick={() => setSelectedDistricts([])} className={`text-xs ${isDark ? 'text-slate-500 hover:text-slate-300' : 'text-gray-400 hover:text-gray-600'}`}>Temizle</button>
                  {selectedDistricts.length > 0 && (
                    <span className={`ml-auto text-xs ${isDark ? 'text-slate-500' : 'text-gray-400'}`}>{selectedDistricts.length} seçili</span>
                  )}
                </div>
                {districts.map((district) => {
                  const checked = selectedDistricts.includes(district)
                  return (
                    <div key={district} onClick={() => toggleDistrict(district)}
                      className={`flex items-center gap-2.5 px-3 py-2 cursor-pointer text-sm transition-colors ${isDark ? checked ? 'bg-blue-500/10 text-white' : 'text-slate-300 hover:bg-slate-800' : checked ? 'bg-blue-50 text-gray-900' : 'text-gray-700 hover:bg-gray-50'}`}>
                      <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors ${checked ? 'bg-blue-600 border-blue-600' : isDark ? 'border-slate-600' : 'border-gray-300'}`}>
        ...
        {selectedDistricts.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-0.5">
            {selectedDistricts.map(d => (
              <span key={d} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs ${isDark ? 'bg-blue-500/15 text-blue-300' : 'bg-blue-50 text-blue-700'}`}>
                {d}
                <button type="button" onClick={() => toggleDistrict(d)} className="hover:opacity-70"><X size={10} /></button>
              </span>
            ))}
          </div>
        )}
      ...
      {loading && progress && (
        <div className="space-y-2 shrink-0">
          ...
          <div className={`w-full h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-slate-800' : 'bg-gray-200'}`}>
            <div className="h-full bg-gradient-to-r from-blue-500 to-blue-400 rounded-full transition-all duration-300 ease-out" style={{ width: `${progressPercent}%` }} />
          </div>
        </div>
      )}
      {loading && !progress && (
        <div className={`flex items-center gap-2.5 px-4 py-2.5 rounded-lg shrink-0 ${isDark ? 'bg-slate-900/60 border border-slate-800/60' : 'bg-white border border-gray-200 shadow-sm'}`}>
      ...
      {/* Ana içerik: harita + liste yan yana değil, üst alta */}
      <div className="flex-1 flex flex-col gap-3 min-h-0">
        ...
        <div className="flex-1 overflow-auto min-h-0">
              ...
              <div className="flex items-center justify-between">
                <p className={`text-xs ${isDark ? 'text-slate-500' : 'text-gray-400'}`}>
                  ...
                  {results.filter(r => r.latitude && r.longitude).length > 0 && (
                    <span className={`ml-2 ${isDark ? 'text-slate-600' : 'text-gray-300'}`}>
                      · {results.filter(r => r.latitude && r.longitude).length} haritada
                    </span>
                  )}
                </p>
                {newCount > 0 && !loading && (
                  <p className="text-xs text-emerald-500 flex items-center gap-1">
                    <Sparkles size={12} />{newCount} yeni kayıt
                  </p>
                )}
              </div>
              {results.map((business, index) => (
                <div key={index} onClick={() => setSelectedBusiness(business)}
                  className={`rounded-lg p-3.5 cursor-pointer transition-all duration-150 ${
                    selectedBusiness === business
                      ? isDark ? 'bg-blue-500/10 border border-blue-500/40' : 'bg-blue-50 border border-blue-200 shadow-sm'
                      : isDark ? 'bg-slate-900/60 border border-slate-800/60 hover:border-slate-700/80 hover:bg-slate-800/40' : 'bg-white border border-gray-200 hover:border-gray-300 hover:shadow-sm shadow-sm'
                  }`}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className={`font-semibold text-sm truncate ${isDark ? 'text-white' : 'text-gray-900'}`}>{business.name}</h3>
                        {business.is_new && <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-medium shrink-0">YENİ</span>}
                        {business.is_new === false && <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 ${isDark ? 'bg-slate-700/50 text-slate-500' : 'bg-gray-100 text-gray-400'}`}>KAYITLI</span>}
                        {!(business.latitude && business.longitude) && (
                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 ${isDark ? 'bg-slate-700/50 text-slate-500' : 'bg-gray-100 text-gray-400'}`}>📍 yok</span>
                        )}
                      </div>
                      {business.address && (
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        {business.social_media && Object.keys(business.social_media).length > 0 && (
                          <span className="flex items-center gap-1 text-pink-500 text-xs">
                            <Share2 size={11} />{Object.keys(business.social_media).length}
                          </span>
                        )}
                      </div>
                    </div>
          ...
          {!loading && results.length === 0 && !error && !showMap && (
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                <Search size={40} className={isDark ? 'text-slate-800 mx-auto' : 'text-gray-200 mx-auto'} />
                <p className={`text-sm mt-3 ${isDark ? 'text-slate-600' : 'text-gray-400'}`}>Arama yapın, işletme verilerini toplayın</p>
                <p className={`text-xs mt-1 ${isDark ? 'text-slate-700' : 'text-gray-300'}`}>Sonuçlar otomatik kaydedilir</p>
              </div>
            </div>
          )}
        </div>
